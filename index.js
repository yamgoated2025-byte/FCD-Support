...
// ─── Message Handler ──────────────────────────────────────────────────────────
const BAD_WORDS = [
  'fuck', 'fck', 'bitch', 'btch', 'b1tch', 'shit', 'sh1t', 'cunt',
  'nigga', 'nigger', 'n1gga', 'n1gger', 'whore', 'slut', 'bastard',
  'cock', 'dick', 'pussy', 'motherfucker', 'asshole',
];
...
client.on('messageCreate', async message => {
  ...
  // ── Automod ───────────────────────────────────────────────────────────────
  const normalized = message.content.toLowerCase().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, ' ').trim();
  ...
  const db = loadDB();
  if (!db.warns[message.author.id]) db.warns[message.author.id] = { count: 0, history: [] };

  db.warns[message.author.id].count += 1;
  db.warns[message.author.id].history.push({ reason: 'Automod: Inappropriate language', timestamp: Date.now() });
  ...
  if (timeoutMs && message.member) {
    await message.member.timeout(timeoutMs, `Automod warn #${warnCount}: Inappropriate language`).catch(() => {});
  }
